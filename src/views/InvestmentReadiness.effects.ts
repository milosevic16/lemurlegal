...
export function initEffects(): () => void {
  ...
  try {
    ...
    (function(){
      ...
      /* ---- 8a. READINESS GAUGE — arc fills + number counts up (on scroll) ---- */
      $('[data-anim="gauge"]').forEach(function(svg){
        var arc = svg.querySelector('[data-anim="gauge-arc"]');
        var num = svg.querySelector('[data-anim="gauge-num"]');
        var score = parseFloat(svg.getAttribute('data-score')) || 0;
        var C = 2 * Math.PI * 84;               // gauge radius 84
        var target = C * (1 - score/100);
        /* initial state set HERE (motion path): empty arc + zero */
        if (arc){ arc.style.strokeDasharray = C; arc.style.strokeDashoffset = C; }
        if (num){ num.textContent = '0'; }
        function run(){
          if (arc){ __fx.anim(arc, [{strokeDashoffset:C},{strokeDashoffset:target}],
            {duration:1650, easing:'cubic-bezier(.2,.7,.2,1)', fill:'forwards'}); }
          if (num){
            var t0=null, dur=1650;
            var tick=function(ts){ if(!t0)t0=ts; var p=Math.min(1,(ts-t0)/dur);
              var e=1-Math.pow(1-p,3); num.textContent=Math.round(score*e);
              if(p<1) requestAnimationFrame(tick); else num.textContent=score; };
            requestAnimationFrame(tick);
          }
        }
        if (!('IntersectionObserver' in window)){ run(); return; }
        var io=new IntersectionObserver(function(es){es.forEach(function(e){ if(e.isIntersecting){ run(); io.unobserve(e.target); } });},{threshold:0.4});
        io.observe(svg);
      });

      /* ---- 8a2. READINESS SCAN — meter bars grow + values count + scanline sweep ---- */
      $('[data-anim="scan"]').forEach(function(panel){
        var rows = $('[data-scan-row]', panel);
        var line = panel.querySelector('[data-scan-line]');
        /* initial states set HERE in JS */
        rows.forEach(function(r){
          var fill=r.querySelector('[data-scan-fill]'); var val=r.querySelector('[data-scan-val]');
          if(fill){ fill.style.transformOrigin='left center'; fill.style.transform='scaleX(0)'; }
        ...
        function run(){
          rows.forEach(function(r,i){
            var fill=r.querySelector('[data-scan-fill]'); var val=r.querySelector('[data-scan-val]');
            var v=parseFloat(r.getAttribute('data-scan-row'))||0; var delay=180+i*140;
            if(fill){ __fx.anim(fill, [{transform:'scaleX(0)'},{transform:'scaleX(1)'}],
              {duration:900, delay:delay, easing:'cubic-bezier(.2,.7,.2,1)', fill:'forwards'}); }
            if(val){ var t0=null,dur=900;
              requestAnimationFrame(function f(ts){ if(!t0)t0=ts+delay; var p=Math.min(1,Math.max(0,(ts-t0)/dur));
                var e=1-Math.pow(1-p,3); val.textContent=Math.round(v*e); if((ts-t0)<dur) requestAnimationFrame(f); else val.textContent=v; });
            }
          });
          if(line){ line.style.opacity='';
            __fx.anim(line, [{top:'-8%',opacity:0},{opacity:.55,offset:.12},{opacity:.55,offset:.88},{top:'100%',opacity:0}],
              {duration:2800, iterations:Infinity, easing:'ease-in-out'}); }
        }
        if (!('IntersectionObserver' in window)){ run(); return; }
        var io=new IntersectionObserver(function(es){es.forEach(function(e){ if(e.isIntersecting){ run(); io.unobserve(e.target); } });},{threshold:0.25});
        io.observe(panel);
      });
    ...
    wireWeb3Form(__fx, { root: '#contact', subject: 'New inquiry — Investment Readiness Review', page: 'Investment Readiness Review' })
