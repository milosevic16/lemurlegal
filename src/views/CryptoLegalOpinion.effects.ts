  ...
  try {
    ...
    (function(){
      ...
      $('[data-anim="cpulse"]').forEach(function(el){
        el.style.cssText = 'position:absolute;top:0;left:-40%;width:40%;height:100%;background:linear-gradient(90deg,transparent,rgba(127,89,245,.8),transparent);pointer-events:none;';
        __fx.anim(el, [
      ...
      $('[data-anim="glitch"]').forEach(function(el){
        ...
        var ov1 = makeOv('#7F59F5'), ov2 = makeOv('#A88BFF');
        function cycle(){
          var period = 3200;
          setTimeout(function(){
            __fx.anim(ov1, [
      ...
      $('[data-anim="mesh"]').forEach(function(cv){
        ...
        function rosette(cx,cy,s,Rr,rr,d,turns,ph,a){
          ...
          ctx.globalAlpha=a; ctx.strokeStyle='rgba(127,89,245,1)'; ctx.lineWidth=0.6; ctx.stroke(); ctx.globalAlpha=1;
        }
        function size(){
      ...
      $('[data-ht-hover-fill]').forEach(function(b){
        ...
        __fx.on(b, 'mouseenter', function(){
          ...
          if (isOutline){ b.style.backgroundColor = '#1A1826'; b.style.color = '#D2DDD7'; }
        });
        __fx.on(b, 'mouseleave', function(){
          ...
          if (isOutline){ b.style.backgroundColor = 'transparent'; b.style.color = '#1A1826'; }
        });
      });
    ...
    wireWeb3Form(__fx, { root: '#contact', subject: 'New inquiry — Crypto Legal Opinion', page: 'Crypto Legal Opinion' })
