...
export function initEffects(): () => void {
  ...
  try {
    ...
    (function(){
      ...
      /* ---- 8. HERO RADAR — rotating sweep + pulsing contact blips + halo ---- */
      $('[data-anim="radar-sweep"]').forEach(function(g){
      ...
      $('[data-anim="radar-blip"]').forEach(function(c){
        var d = parseFloat(c.getAttribute('data-delay')) || 0;
        c.style.opacity = '0';
        __fx.anim(c, [
          {opacity:0, transform:'scale(0.4)'},
          {opacity:1, transform:'scale(1)', offset:.12},
          {opacity:.15, transform:'scale(1)', offset:.6},
          {opacity:0, transform:'scale(0.4)'}
        ], {duration:5200, delay:d, iterations:Infinity, easing:'ease-out'});
      });
      ...
      /* ---- 8c. CLASSIFICATION TRACE — sequential row reveal + decrypt status tokens ---- */
      $('[data-anim="qlog"]').forEach(function(box){
        var rows = $('[data-qlog-row]', box);
        var cursor = box.querySelector('[data-qlog-cursor]');
        var CH = "ABCDEFGHJKLMNPQRSTUVWXYZ0123456789/#·";
        /* initial hidden states set HERE in JS */
        rows.forEach(function(r){ r.style.opacity='0'; r.style.transform='translateY(6px)'; });
        if (cursor){ cursor.style.opacity='0'; }
        function decrypt(el, dur){
          var fin = el.getAttribute('data-final') || el.textContent; var t0=null;
          requestAnimationFrame(function f(ts){ if(!t0)t0=ts; var p=Math.min(1,(ts-t0)/dur);
            var n=fin.length, rev=Math.floor(p*n), out='';
            for(var i=0;i<n;i++){ var ch=fin.charAt(i);
              out += (i<rev || ch===' ') ? ch : CH.charAt(Math.floor(Math.random()*CH.length)); }
            el.textContent=out; if(p<1) requestAnimationFrame(f); else el.textContent=fin; });
        }
        function run(){
          rows.forEach(function(r,i){
            var delay = 160 + i*340;
            __fx.anim(r, [{opacity:0, transform:'translateY(6px)'},{opacity:1, transform:'none'}],
              {duration:300, delay:delay, easing:'ease', fill:'forwards'});
            var val = r.querySelector('[data-qlog-val]');
            if (val){ setTimeout(function(){ decrypt(val, 520); }, delay+180); }
          });
          if (cursor){
            var total = 160 + rows.length*340;
            setTimeout(function(){ cursor.style.opacity='';
              __fx.anim(cursor, [{opacity:1},{opacity:0}],{duration:900, iterations:Infinity, easing:'steps(1)'}); }, total);
          }
        }
        if (!('IntersectionObserver' in window)){ run(); return; }
        var io=new IntersectionObserver(function(es){es.forEach(function(e){ if(e.isIntersecting){ run(); io.unobserve(e.target); } });},{threshold:0.3});
        io.observe(box);
      });
    ...
    wireWeb3Form(__fx, { root: '#contact', subject: 'New inquiry — Trade Regulatory Qualification', page: 'Trade Regulatory Qualification' })
